<h1 align="left">Álvaro d'Ors Nestares</h1>
<h3 align="left">3rd-year Computer Engineering • Backend & Automation Enthusiast</h3>

---

## About Me
I’m a Computer Engineering student passionate about backend development, automation, and developer tools.  
I focus on building systems that simplify workflows, automate repetitive tasks, and extract meaningful insights from data, with clarity and maintainability in mind.

Currently working on:

- [Gideon](https://github.com/AlvarodOrs/Gideon) - an intelligent automation assistant for Git and Docker workflows  
- [Uriel](https://github.com/AlvarodOrs/Uriel) - an OSInt tool for collecting and analyzing open-source intelligence
- Python Selenium bots - automation of numerous processes

I’m also actively improving my Java skills, exploring backend architectures, and studying scalable, agentic workflows.

---
## 📬 Connect

Connect with me on [LinkedIn](https://www.linkedin.com/in/dors-alvaro) or send me a message via [this form](https://formspree.io/f/xdkqypkz) 

---
<h1>📊 GitHub Stats
<img align="right" src="https://komarev.com/ghpvc/?username=alvarodors&label=Profile%20views&color=blueviolet&style=flat-square"/>
</h1>
<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=AlvarodOrs&theme=ambient_gradient&hide_border=false&count_private=true&include_all_commits=true"/>
  <img src="https://nirzak-streak-stats.vercel.app/?user=AlvarodOrs&theme=ambient_gradient&hide_border=false"/><br/>
</p>

<!-- # 🏆 GitHub Trophies
<p align="center">
  <img src="https://github-profile-trophy.vercel.app/?username=AlvarodOrs&theme=ambient_gradient&margin-w=4"/>
</p> -->

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=AlvarodOrs&theme=ambient_gradient&hide_border=false&include_all_commits=true&count_private=true&layout=compact"/>
  <img src="https://github-contributor-stats.vercel.app/api?username=AlvarodOrs&limit=5&theme=ambient_gradient&combine_all_yearly_contributions=true"/>
</p>

---

# 🛠 Technical Skills

## Programming & Scripting
Python  |  C  |  C++  |  Java  |  JavaScript  |  TypeScript

## Systems, Servers & DevOps
Linux  |  Windows OS  |  Bash  |  Git  |  Docker  |  Nginx

## Web Development
HTML5  |  CSS3  |  PHP

## Tools, Modeling & Hardware
Arduino  |  Raspberry Pi